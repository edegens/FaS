...
func main() {
	src := loadImage("testdata/src.jpeg")
	...
	for name, filter := range filters {
		wg.Add(1)
		go func(name string, filter gift.Filter, wg *sync.WaitGroup) {
			log.Println("Applying filter: ", name)
			g := gift.New(filter)
			dst := image.NewNRGBA(g.Bounds(src.Bounds()))
			g.Draw(dst, src)
			saveImage("testdata/dst_"+name+".jpeg", dst)
			log.Println("Done applying filter: ", name)
			wg.Done()
		}(name, filter, &wg)
	}
	wg.Wait()
